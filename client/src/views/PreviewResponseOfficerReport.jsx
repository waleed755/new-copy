...
import { Descriptions, Divider, List, Button, Input, Select, Form, message, Image, DatePicker, Upload, Modal } from 'antd';
import { CheckOutlined, PlusOutlined, EditOutlined, UploadOutlined } from '@ant-design/icons';
import { downloadActivityInviteApi, editBranchApi, getSingleOfficerResponseApi, resetOfficerResponseALreadyFilledApi, updateActivityStatusApi, updateOfficerResponseALreadyFilledApi, updateOfficerResponseApi } from '../services/apiConstants';
...
import ActivityLogo from "../assets/512-by-512.png";

import { Document, Page, Text, Image as PdfImage, View, StyleSheet, Link } from '@react-pdf/renderer';
...
export const PreviewResponseOfficerReport = ({ activity, onResetSuccess }) => {
  ...
  const [propertyAIValue, setPropertyAIValue] = useState({});
  const [propertyAIImages, setPropertyAIImages] = useState([]);
  const [modalVisible, setModalVisible] = useState(false);
  const [inputValue, setInputValue] = useState('');
  const [newInputValue, setNewInputValue] = useState('');
  
  const latitude = 31.49; // Replace with your actual latitude value
  const longitude = 74.34; // Replace with your actual longitude value

  const [addBranchLoading, setAddBranchLoading] = useState(false);
  const modalTitle = 'Send';
  ...
  const MyDocument = React.memo(({ activity, data, responseFields, officerPhotos }) => (
    <Document>
      <Page wrap size="A4" style={styles.page}>
        ...
        {activity.activityOnSiteDate && ( <View style={styles.section} >
        ...
        <View style={styles.footer} fixed>
         <View style={{display:'flex',gap:6,flexDirection:'row' ,alignItems:'center'}}>
          <PdfImage src={Logo} fixed alt="Logo" style={{ width: 80 }} />
         <Text style={{marginTop:3}}> rapto.uk</Text>
         </View>
  ...
  const fetchData = async () => {
    const response = await getSingleOfficerResponseApi({ activityId: activity?._id });
    if (response.data.success) {
      setData(response.data.officerResponse);
      setPropertyAIValue(response.data.propertyAI);
      setPropertyAIImages(response.data.aiImages);
      setNewInputValue(response.data.branchEmail);
      form.setFieldsValue({
  ...
  const getData = async () => {
    const fetchRows = await getSingleOfficerResponseApi({ activityData: { activityId: activity?._id } });
    if (fetchRows.data.success) {
      setData(fetchRows?.data?.officerResponse);
      setPropertyAIValue(fetchRows.data.propertyAI);
      setPropertyAIImages(fetchRows.data.aiImages);
      setNewInputValue(fetchRows.data.branchEmail);
  ...
  const handleResetOfficerResponse = async () => {
    try {
      await resetOfficerResponseALreadyFilledApi({ officerResponseId: data?._id },data?._id);
      onResetSuccess(true); // Notify parent about the success
    } catch (error) {
      console.error("Error resetting officer response:", error);
    }
  };
  ...
  const toggleFieldEditability = useCallback(() => {
  ...
  }, [officerResponse, activity, data?._id, navigate]);

  const handleBranchModal = useCallback(() => {
  ...
  const handleModalOk = useCallback(async () => {
    try {
      ...
      if (!inputValue) {
        const branch = {
          branchData: {
            _id: activity?.branchId,
            branchEmail: newInputValue
          }
        };
        await editBranchApi(branch);
      }
      const response = await downloadActivityInviteApi({ activityId: activity?._id });
      if (response.data) {
      ...
      setActivityLoading(false);
      message.error('Email has not sent');
    }
  }, [activity?._id, newInputValue, inputValue]);
  ...
  return (
    <div className=''>
      {isEditing ? (
        ...
        {!modalVisible &&  <PDFDownloadLink
              document={<MyDocument activity={activity} data={data} responseFields={responseFields} officerPhotos={officerPhotos} />}
              fileName="report.pdf"
            >
              {({ loading }) =>
          ...
          <Button type='primary' size='large' loading={activityloading} onClick={handleBranchModal}>
            Send Report
          </Button>
          <Modal
        title={'Send Report'}
        ...
        onCancel={() => [setModalVisible(false),            setNewInputValue('')
        ]}
        confirmLoading={activityloading}
      >
        <Input placeholder={`Enter recipient's email address (use commas to separate multiple addresses)`} value={newInputValue} onChange={handleBranchInputChange} />
      </Modal>       </div>)
      )}
            ...
            <div className='m-5' id="pdf-content">
              ...
              <Descriptions bordered >
                ...
                <Descriptions.Item label='Property AI'>
                  {propertyAIValue?.value ? propertyAIValue?.value : '-'}
                </Descriptions.Item>
      ...
      {isResponseEditing ? (
        ...
        <>
          ...
          />
            <Button
        type='primary'
        onClick={() => handleResetOfficerResponse()}
      >
        Want to fill form
      </Button>
        </>
...
export default PreviewResponseOfficerReport;
